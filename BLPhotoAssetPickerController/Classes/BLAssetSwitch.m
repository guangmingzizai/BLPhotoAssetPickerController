...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self == [super initWithFrame:frame]) {
        ...
        CGSize labelSize = titleRect.size;
        ...
        [_titleView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.left.mas_equalTo(self.frame.size.width/2 - (titleRect.size.width+17)/2);
...
- (void)updateConstraintsWithString:(NSString *)str {
    ...
    [_titleView mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(CGSizeMake(titleRect.size.width + 3 + 14, 44));
        make.left.mas_equalTo(self.frame.size.width/2 - (titleRect.size.width+17)/2);
        make.top.mas_equalTo(0);
    }];
